...
class GoogleSheetsIntegration {
  ...
  async loadConfig() {
    try {
      const response = await fetch("/api/assets?asset=config.json");
      if (!response.ok)
...
document.addEventListener("DOMContentLoaded", () => {
  const a = new AdminDashboard();
  window.adminDashboard = a;
});
